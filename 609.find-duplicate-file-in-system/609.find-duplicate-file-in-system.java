    ...
    public List<List<String>> findDuplicate(String[] paths) {
        ...
        for (String s : paths) {
            ...
            for (int i = 1; i < files.length; i++) {
                ...
                res.computeIfAbsent(files[i].substring(p), v -> new ArrayList<>()).add(files[0] + "/" + files[i].substring(0, p));
            }
        }

        return res.values().stream().filter(list -> list.size() > 1).collect(Collectors.toList());
    }
}
// @lc code=end

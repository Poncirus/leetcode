...
class Solution:
    def accountsMerge(self, accounts: List[List[str]]) -> List[List[str]]:
        ...
        for account in accounts:
            name = account[0]
            email = set(account[1:])
            if name not in names_emails:
                names_emails[name] = [email]
                continue
            added_emails = names_emails[name]
            new_emails = []
            for added_email in added_emails:
                if added_email & email:
                    email.update(added_email)
                else:
                    new_emails.append(added_email)
            new_emails.append(email)
            names_emails[name] = new_emails
        res = []
        for name, emails in names_emails.items():
            for email in emails:
